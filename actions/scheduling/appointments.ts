...
import { CreateAppointmentSchema, AppointmentCheckinSchema, AppointmentRecommendationSchema, AppointmentReviewSchema } from "@/lib/validation/schemas"
...
const ALLOWED_TRANSITIONS: Record<AppointmentStatus, AppointmentStatus[]> = {
  ...
  rejected: [], scheduled: ["reminded", "checked_in", "cancelled", "no_show"],
  reminded: ["checked_in", "cancelled", "no_show"], checked_in: ["in_consultation", "cancelled"],
  in_consultation: ["completed"], completed: [], cancelled: [], no_show: [],
}

export async function submitAppointmentRequest(input: unknown): Promise<ActionResult<AppointmentDTO>> {
  ...
  const rate = checkRateLimit(`appointment:${actor.id}`, 3, 24 * 60 * 60 * 1000)
  if (!rate.success) return { success: false, error: "Appointment request limit reached", code: "RATE_LIMIT" }
  ...
  const candidate = isObject(input) ? { ...input, patient_type: actor.role, [profileIdField]: profile.id } : input
  const parsed = CreateAppointmentSchema.safeParse(candidate)
  if (!parsed.success) return { success: false, error: "Invalid appointment request", code: "VALIDATION" }
...
export async function evaluateAppointmentRequest(appointmentId: string): Promise<ActionResult<null>> {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["admin", "nurse", "doctor"])) return forbidden()
  const admin = createAdminClient()
  const { data: appointment } = await admin
    ...
    .eq("id", appointmentId)
    .maybeSingle()
  if (!appointment) return { success: false, error: "Appointment not found", code: "NOT_FOUND" }
  if (appointment.status !== "pending") return { success: false, error: "Appointment cannot be evaluated in its current state", code: "INVALID_STATE" }

  const result = await evaluateAppointmentWithAI({ actorId: actor.id, appointmentId, reason: appointment.reason, symptoms: appointment.symptoms })
  ...
  await admin.from("appointments").update({ priority: result.evaluation.priority_score, status: "ai_evaluated" }).eq("id", appointmentId)
  await writeAuditLog(actor, "appointment.ai_evaluated", "appointment", appointmentId, { ai_status: result.status })
  return { success: true, data: null }
}

export async function recommendAppointment(input: unknown): Promise<ActionResult<null>> {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["doctor", "nurse"])) return forbidden()
  const parsed = AppointmentRecommendationSchema.safeParse(input)
  if (!parsed.success) return { success: false, error: "Invalid recommendation", code: "VALIDATION" }
  const admin = createAdminClient()
  const { data: appointment } = await admin.from("appointments").select("status").eq("id", parsed.data.appointment_id).maybeSingle()
  if (!appointment) return { success: false, error: "Appointment not found", code: "NOT_FOUND" }
  if (!canTransition(appointment.status as AppointmentStatus, "recommended")) return invalidTransition()
  const { error } = await admin.from("appointments").update({ status: "recommended" }).eq("id", parsed.data.appointment_id)
  if (error) return { success: false, error: "Unable to save recommendation", code: "DATABASE" }
  await admin.from("appointment_recommendations").insert({ appointment_id: parsed.data.appointment_id, recommended_by: actor.id, recommendation: parsed.data.recommendation, notes: parsed.data.notes ?? null, recommended_date: parsed.data.recommended_date ?? null, recommended_time: parsed.data.recommended_time ?? null })
  await writeAuditLog(actor, "appointment.recommended", "appointment", parsed.data.appointment_id, { recommendation: parsed.data.recommendation })
  return { success: true, data: null }
}

/** Nurse/admin decision. Scheduling details are deliberately required for approval. */
export async function reviewAppointment(input: unknown): Promise<ActionResult<null>> {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["admin", "nurse"])) return forbidden()
  const parsed = AppointmentReviewSchema.safeParse(input)
  if (!parsed.success) return { success: false, error: "Invalid review", code: "VALIDATION" }
  const admin = createAdminClient()
  const { data: appointment } = await admin
    .from("appointments")
    ...
    .maybeSingle()
  if (!appointment) return { success: false, error: "Appointment not found", code: "NOT_FOUND" }
  if (!canTransition(appointment.status as AppointmentStatus, parsed.data.decision === "approved" ? "scheduled" : "rejected")) return invalidTransition()

  if (parsed.data.decision === "rejected") {
    const { error } = await admin.from("appointments").update({ status: "rejected" }).eq("id", appointment.id)
    if (error) return { success: false, error: "Unable to reject appointment", code: "DATABASE" }
    await writeAuditLog(actor, "appointment.rejected", "appointment", appointment.id, { reason: parsed.data.rejection_reason ?? null })
    return { success: true, data: null }
  }
  return { success: false, error: "Use scheduleApprovedAppointment to schedule an approved request", code: "SCHEDULE_REQUIRED" }
}

export async function scheduleApprovedAppointment(input: unknown): Promise<ActionResult<null>> {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["admin", "nurse"])) return forbidden()
  const parsed = ScheduleSchema.safeParse(input)
  if (!parsed.success) return { success: false, error: "Invalid schedule", code: "VALIDATION" }
  const admin = createAdminClient()
  const { data: appointment } = await admin
    .from("appointments")
    .select("id, status, patient_type, student_id, faculty_id")
    .eq("id", parsed.data.appointment_id).maybeSingle()
  if (!appointment) return { success: false, error: "Appointment not found", code: "NOT_FOUND" }
  if (!canTransition(appointment.status as AppointmentStatus, "scheduled")) return invalidTransition()
  ...
  if (error) return { success: false, error: "Unable to schedule appointment", code: "DATABASE" }
  const receiverId = await getPatientUserId(admin, appointment.patient_type, appointment.student_id, appointment.faculty_id)
  if (receiverId) {
    const scheduledAt = new Date(`${parsed.data.scheduled_date}T${parsed.data.scheduled_time}`)
    await admin.from("appointment_reminders").insert([
      { appointment_id: appointment.id, remind_at: new Date(scheduledAt.getTime() - 24 * 60 * 60 * 1000).toISOString() },
      { appointment_id: appointment.id, remind_at: new Date(scheduledAt.getTime() - 60 * 60 * 1000).toISOString() },
    ])
    await sendNotification(actor, { receiver_id: receiverId, title: "Appointment scheduled", message: `Your appointment is scheduled for ${parsed.data.scheduled_date} at ${parsed.data.scheduled_time}.`, type: "appointment", entity_type: "appointment", entity_id: appointment.id })
  }
  await writeAuditLog(actor, "appointment.scheduled", "appointment", appointment.id)
  return { success: true, data: null }
}

export async function checkInAppointment(input: unknown): Promise<ActionResult<null>> {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["admin", "nurse", "doctor"])) return forbidden()
  const parsed = AppointmentCheckinSchema.safeParse(input)
  if (!parsed.success) return { success: false, error: "Invalid check-in", code: "VALIDATION" }
  const admin = createAdminClient()
  const { data: appointment } = await admin.from("appointments").select("status").eq("id", parsed.data.appointment_id).maybeSingle()
  if (!appointment) return { success: false, error: "Appointment not found", code: "NOT_FOUND" }
  if (!canTransition(appointment.status as AppointmentStatus, "checked_in")) return invalidTransition()
  const { error } = await admin.from("appointment_checkins").insert({ appointment_id: parsed.data.appointment_id, rfid_uid: parsed.data.rfid_uid ?? null })
  if (error) return { success: false, error: "Unable to record check-in", code: "DATABASE" }
  await admin.from("appointments").update({ status: "checked_in" }).eq("id", parsed.data.appointment_id)
  await writeAuditLog(actor, "appointment.checked_in", "appointment", parsed.data.appointment_id)
  return { success: true, data: null }
}
...
function canTransition(from: AppointmentStatus, to: AppointmentStatus): boolean { return ALLOWED_TRANSITIONS[from]?.includes(to) ?? false }
function invalidTransition(): ActionResult<never> { return { success: false, error: "Appointment cannot transition from its current state", code: "INVALID_STATE" } }
function forbidden(message = "Forbidden"): ActionResult<never> { return { success: false, error: message, code: "FORBIDDEN" } }
async function getPatientUserId(admin: ReturnType<typeof createAdminClient>, patientType: string, studentId: string | null, facultyId: string | null): Promise<string | null> {
