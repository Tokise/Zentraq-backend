...
export async function registerStudentProfileAction(formData: FormData) {
...
export async function lookupStudentByRfidAction(formData: FormData) {
  const actor = await requireAdmin();
...
export async function generateStudentIdAction() {
  const actor = await requireAdmin();
  if (!actor) return { error: "Unauthorized" };
