...
export async function evaluateAppointmentRequestAction(
...
export async function recommendAppointmentAction(
  input: unknown,
): Promise<ActionResult<null>> {
  const actor = await getActionActor();
...
export async function reviewAppointmentAction(
  input: unknown,
): Promise<ActionResult<null>> {
  const actor = await getActionActor();
  ...
  return {
    success: false,
    error: "Use scheduleApprovedAppointmentAction to schedule an approved request",
    code: "SCHEDULE_REQUIRED",
  };
}

export async function scheduleApprovedAppointmentAction(
  input: unknown,
): Promise<ActionResult<null>> {
  const actor = await getActionActor();
  if (!actor || !hasAnyRole(actor, ["admin", "nurse"])) return forbidden();
...
export async function getMyAppointmentsAction(): Promise<{
  error: string | null;
