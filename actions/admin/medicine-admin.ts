...
export async function getRestockRequestsAction(): Promise<{
  error: string | null
  requests: Array<{
    id: string
    medicine_id: string
  ...
  const actor = await staff(["admin", "doctor", "nurse"])
  if (!actor) return { error: "Access denied", requests: [] }

  const admin = createAdminClient()
  const { data, error } = await admin
    .from("restock_requests")
    .select(`
      id, medicine_id, quantity, status, created_at, updated_at,
      medicines(generic_name, brand_name)
    `)
...
export async function updateRestockRequestAction(id: string, status: "approved" | "rejected" | "fulfilled"): Promise<{ success?: boolean; error?: string }> {
  const actor = await staff(["admin"])
  if (!actor) return { error: "Access denied - only admins can update restock requests" }
