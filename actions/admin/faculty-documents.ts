...
export async function uploadFacultyDocumentAction(formData: FormData) {
  const actor = await getActionActor()
  if (!actor || !hasAnyRole(actor, ["admin", "nurse"])) return { error: "Access denied" }
  const facultyId = String(formData.get("facultyId") ?? "")
  const documentType = String(formData.get("documentType") ?? "")
  const file = formData.get("file")
  if (!facultyId || !documentType || !(file instanceof File) || file.size === 0) {
    return { error: "Faculty member, document type, and file are required" }
  }
  if (!new Set(["application/pdf", "image/jpeg", "image/png"]).has(file.type) || file.size > MAX_SIZE) {
  ...
  const admin = createAdminClient()
  const { error: uploadError } = await admin.storage.from(BUCKET).upload(path, await file.arrayBuffer(), { contentType: file.type, upsert: false })
