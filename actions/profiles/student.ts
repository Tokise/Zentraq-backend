...
export async function getStudentProfileDTOAction() {
  try {
    const auth = await requireStudentUser()
    if (auth.error || !auth.user) {
    ...
    if (error) {
      console.error("[getStudentProfileDTOAction DB Error]:", error)
    ...
    console.error("[getStudentProfileDTOAction Exception]:", err)
