...
export async function downloadAggregateReport(
  ...
  while (Date.now() - startedAt < REPORT_TIMEOUT_MS) {
    ...
    const status = await getAggregateReportStatusAction(request.requestId)
    if (status.error || !status.report) {
      throw new Error(status.error ?? "The report status is unavailable.")
    }
    if (status.report.status === "dead_letter") {
      throw new Error("The report could not be generated.")
    }
    if (status.report.status !== "completed") continue

    const download = await getAggregateReportDownloadUrlAction(
      request.requestId,
    )
    if (download.error || !download.url) {
      throw new Error(download.error ?? "The report download is unavailable.")
    }
    window.location.assign(download.url)
    return
  }

  throw new Error("The report is still processing. Try again shortly.")
}
