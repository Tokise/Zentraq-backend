...
export async function uploadProfilePhoto(
  ...
  if (!match) {
    return {
      error: "Profile photos must be JPEG, PNG, or WebP images.",
      path: null,
    };
  }
...
// Resolves private profile-photo paths in one Storage request while preserving order.
export async function resolveProfilePhotoUrls(
    ...
    new Set(
      values.filter(
        (value): value is string =>
          Boolean(value && isPrivateProfilePhotoPath(value)),
      ),
    ),
  );
  if (privatePaths.length === 0) return values;

  const signed = await admin.storage
    .from(PROFILE_PHOTO_BUCKET)
    .createSignedUrls(privatePaths, 5 * 60);
  if (signed.error) {
    return values.map((value) =>
      value && isPrivateProfilePhotoPath(value) ? null : value
    );
  }
  ...
  return values.map((value) =>
    value && isPrivateProfilePhotoPath(value)
      ? (signedByPath.get(value) ?? null)
      : value
  );
}
...
function isPrivateProfilePhotoPath(value: string): boolean {
  return /^(student|faculty|staff)\/[0-9a-f-]{36}\/[0-9a-f-]{36}\.(jpg|png|webp)$/i
    .test(
      value,
    );
}
