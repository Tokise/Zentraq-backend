...
export const CreateStudentSchema = z.object({
  ...
  email: z.string().email("Invalid email format").max(255).optional().or(
    z.literal(""),
  ),
...
export const FinalizeConsultationWorkflowSchema = z.object({
  ...
  if (
    value.vitals_disposition === "not_required" && !value.vitals_skip_reason
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (
    value.vitals_disposition === "required" &&
    Object.keys(value.vitals).length === 0
  ) {
    context.addIssue({
      code: z.ZodIssueCode.custom,
      path: ["vitals"],
...
export const CreateAppointmentSchema = z.object({
  ...
  (data) =>
    (data.patient_type === "student" && data.student_id) ||
...
export const CreateIncidentSchema = z.object({
  ...
  {
    message:
      "Either student_id or faculty_id is required based on patient_type",
    path: ["patient_type"],
  },
);
...
export const CreateHealthClearanceSchema = z.object({
  ...
  (data) =>
    (data.requester_type === "student" && data.student_id) ||
    (data.requester_type === "faculty" && data.faculty_id) ||
    (data.requester_type === "staff" && data.faculty_id),
  {
    message:
      "Either student_id or faculty_id is required based on requester_type",
    path: ["requester_type"],
  },
);
...
export const ClearanceRequestSchema = z.object({
  ...
  status: z.enum(["submitted", "processing", "evaluated", "completed"]).default(
    "submitted",
  ),
});
...
export const IssueCertificateSchema = z.object({
  clearance_id: UUIDSchema,
  certificate_number: z.string().min(1, "Certificate number is required").max(
    100,
  ),
});
...
export type CreateTriageAssessmentInput = z.infer<
  typeof CreateTriageAssessmentSchema
>;
...
export type CreateHealthClearanceInput = z.infer<
  typeof CreateHealthClearanceSchema
>;
export type CreateHealthProgramInput = z.infer<
  typeof CreateHealthProgramSchema
>;
