...
export async function submitClearanceRequest(data: {
  requester_type: "student" | "faculty"
  requester_id: string
  purpose: string
}) {
...
export async function getClearanceStatus(requesterId: string, requesterType: "student" | "faculty") {
  const actor = await getActionActor()
  if (!actor) return { error: "Access denied", clearances: [] as Clearance[] }
...
export async function rejectClearance(clearanceId: string, reason: string) {
  const actor = await staff(["admin"])
...
export async function getClearanceDetail(clearanceId: string) {
  const actor = await staff(["admin", "doctor", "nurse"])
