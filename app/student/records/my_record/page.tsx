...
export default function StudentRecordsMyRecordPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await getPatientMedicalRecord("self", "student")
      if (res.error) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="My Medical Record" description="Your health record information." />
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-muted-foreground">No medical record found.</p>
            </div>
          ) : (
