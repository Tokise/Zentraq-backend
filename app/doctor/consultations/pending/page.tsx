...
export default function DoctorConsultationsPendingPage() {
  ...
  const fetchConsultations = async () => {
    setLoading(true)
    try {
      const result = await getConsultationQueue(["pending", "in_progress"])
      if (result.error) {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto mt-[-25px] px-4 py-4">
      <PageHeader
        ...
        description="Review and manage patient consultations."
      />
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-14 px-6 text-center space-y-1.5">
              ...
              <p className="text-sm font-medium text-zinc-600">No pending consultations</p>
              <p className="text-xs text-zinc-400">New consultations will appear here.</p>
            </div>
          ) : (
            <>
              <Table>
