...
import { getClearanceQueue } from "@/app/actions/workflow-queries"
import { toast } from "sonner"
import { useRouter } from "next/navigation"

interface QueueClearance {
  id: string
...
export default function DoctorClearancesRequestsPage() {
  ...
  const [clearances, setClearances] = useState<QueueClearance[]>([])
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return clearances
    return clearances.filter((c) => {
      return c.requester_type.toLowerCase().includes(q) || (c.purpose || "").toLowerCase().includes(q) || c.status.toLowerCase().includes(q)
    })
  }, [clearances, search])
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto mt-[-25px] px-4 py-4">
      <PageHeader
        ...
        description="Review and evaluate health clearance requests."
      />
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-14 px-6 text-center space-y-1.5">
              <FileCheck className="size-7 text-zinc-300 mx-auto" />
              <p className="text-sm font-medium text-zinc-600">No clearance requests</p>
              <p className="text-xs text-zinc-400">New clearance requests will appear here.</p>
            </div>
          ) : (
            <>
              <Table>
                ...
                <TableBody>
                  {paginated.map((c) => (
                    <TableRow
                      ...
                      onClick={() => router.push(`/doctor/clearances/${c.id}`)}
                    >
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <User className="size-3.5 text-zinc-400" />
                          <span className="text-sm font-medium capitalize">{c.requester_type}</span>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm text-zinc-600">{c.purpose || "—"}</TableCell>
