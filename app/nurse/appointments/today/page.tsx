...
export default function NurseAppointmentsTodayPage() {
  ...
  const fetchAppointments = async () => {
    setLoading(true)
    try {
      const result = await getAppointmentQueue(["approved", "confirmed"])
      if (result.error) {
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-14 px-6 text-center space-y-1.5">
              ...
              <p className="text-sm font-medium text-zinc-600">No appointments today</p>
              <p className="text-xs text-zinc-400">Scheduled appointments will appear here.</p>
            </div>
          ) : (
            <>
              <Table>
