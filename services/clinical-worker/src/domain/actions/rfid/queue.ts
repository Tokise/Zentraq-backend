import { createAdminClient } from "../../../runtime/context.js";
import { getRfidRolloutStatus } from "../../lib/serverless/rfid-rollout.js";
import { resolveProfilePhotoUrls } from "../../lib/storage/profile-photos.js";
import { firstRelation, requireClinicStaff } from "./shared.js";
...
export async function getRfidServerlessDiagnosticAction(): Promise<{
  ...
  return {
    ...
    diagnostic: {
      ...
      lastExecutionPath: executionPath === "edge" || executionPath === "legacy"
        ? executionPath
        : null,
...
type QueueVisitRow = {
  ...
  consultations:
    | {
      id: string;
      review_doctor_id: string | null;
      status: RfidQueueItem["consultationStatus"];
    }
    | Array<{
      id: string;
      review_doctor_id: string | null;
...
export async function getRfidQueueAction(): Promise<{
  ...
  let queueQuery = admin
    ...
    .select(
      "id, status, priority, created_at, claimed_by, clinic_visits(patient_type, student_id, faculty_id, staff_id, consultations(id, review_doctor_id, status), students(user_id, first_name, last_name, profile_photo_url), faculty(user_id, first_name, last_name, profile_photo_url), staff(user_id, first_name, last_name, profile_photo_url))",
    )
    .in("status", ["waiting", "claimed", "awaiting_doctor_review"]);
  if (auth.role !== "admin") {
    queueQuery = queueQuery.or(
      `claimed_by.is.null,claimed_by.eq.${auth.user.id}`,
    );
  }
  ...
  const queue = rows
    ...
    .flatMap((item): RfidQueueItem[] => {
      ...
      return [{
        ...
        consultationStatus: consultation?.status ?? "queued",
        patientId: (
        ...
        canStartConsultation: item.status === "claimed" &&
          (consultation?.status === "claimed" ||
  ...
  const signedPhotoUrls = await resolveProfilePhotoUrls(
    admin,
    queue.map((item) => item.profilePhotoUrl),
  );
  const queueWithSignedPhotos = queue.map((item, index) => ({
    ...item,
    profilePhotoUrl: signedPhotoUrls[index] ?? null,
  }));
...
function canAccessQueueItem(
  ...
  if (item.claimed_by) return role === "admin" && item.status === "claimed";
