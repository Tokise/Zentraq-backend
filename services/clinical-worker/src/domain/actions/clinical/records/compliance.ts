import { revalidatePath } from "../../../../runtime/effects.js";
import { z } from "zod";
...
import { resolveProfilePhotoUrl } from "../../../lib/storage/profile-photos.js";
...
export async function getComplianceRecordAction(input: unknown): Promise<{
  ...
  return {
    ...
    record: {
      ...
      employeeStatus: parsed.data.patientRole === "student"
        ? null
...
export async function uploadMedicalExamAction(
  ...
  const { error: insertError } = await admin
    ...
    .insert({
      ...patientColumns,
      school_year: parsed.data.patientRole === "student"
        ? parsed.data.schoolYear
        : null,
      semester: parsed.data.patientRole === "student"
        ? parsed.data.semester
        : null,
      calendar_year: parsed.data.patientRole === "student"
        ? null
        : parsed.data.calendarYear,
      clinical_result_status: parsed.data.patientRole === "student"
...
export async function uploadGeneralDocumentAction(
  ...
  const idColumn = `${parsed.data.patientRole}_id`;
  const storagePath = `${parsed.data.patientId}/${crypto.randomUUID()}.` +
...
export async function uploadSickLeaveAction(
  ...
  if (!(file instanceof File)) {
    return { error: "A certificate file is required" };
  }
...
> {
  ...
  const [student, faculty, staff] = await Promise.all([
    admin.from("students").select(select.student).eq("user_id", userId)
      .maybeSingle(),
    admin.from("faculty").select(select.employee).eq("user_id", userId)
      .maybeSingle(),
    admin.from("staff").select(select.employee).eq("user_id", userId)
      .maybeSingle(),
  ]);
  if (student.data) {
...
async function loadClinicalData(
  ...
  const baseError = historyResult.error?.message ??
...
async function loadComplianceData(
  ...
  const examPromise = admin
    ...
    .eq(idColumn, patientId)
    .order("created_at", { ascending: false });
  const sickPromise = patientRole === "student"
  ...
  const error = examResult.error?.message ?? sickResult.error?.message ??
...
async function loadAggregatedDocuments(
  ...
  const [genericResult, examResult, sickResult] = await Promise.all([
    admin
      .from(genericTable)
      .select(
        "id,document_type,file_url,file_name,mime_type,file_size,created_at",
      )
      .eq(idColumn, patientId)
      .order("created_at", { ascending: false }),
    admin
  ...
  const error = genericResult.error?.message ??
...
async function signCompliancePaths(
  paths: string[],
...
function validateExamRoleFields(
...
function validateComplianceFile(
...
function buildStoragePath(
  ...
  return `${patientRole}/${patientId}/${category}/${crypto.randomUUID()}.${
