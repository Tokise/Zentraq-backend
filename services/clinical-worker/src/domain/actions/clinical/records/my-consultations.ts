...
import { getActionActor } from "../../../../runtime/context.js";
  ...
  const accountIds = [
    ...new Set(
      consultations.flatMap((consultation) => {
        const id = consultation.completed_by_clinic_account_id ??
...
async function loadOverviewCounts(
  ...
  return {
    ...
    diagnosesAndTreatments: (diagnoses.count ?? 0) + (treatments.count ?? 0),
...
async function loadTabPage(
  ...
  const table = tab === "prescriptions"
    ...
    : null;
  if (table) {
    return await loadSingleTablePage(
      table,
  ...
  return await loadDiagnosisTreatmentPage(
    consultationIds,
    consultationMap,
    clinicians,
    start,
...
async function loadSingleTablePage(
  ...
  const select = tab === "prescriptions"
...
function clinicianFor(
  ...
  const accountId = consultation.completed_by_clinic_account_id ??
    consultation.doctor_id ??
    consultation.nurse_id;
