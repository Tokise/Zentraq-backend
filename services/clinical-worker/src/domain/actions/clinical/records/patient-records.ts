...
export async function getPatientMedicalRecordAction(
  patientId: string,
  patientType: "student" | "faculty",
) {
  ...
  if (!actor) {
    return {
      error: "Access denied",
      record: null as PatientMedicalRecord | null,
      records: [] as PatientMedicalRecord[],
    };
  }
...
export async function getOwnMedicalRecordAction() {
  const actor = await getActionActor();
  if (!actor) {
    return {
  ...
  if (actor.role === "student") {
    ...
    if (!studentData) {
      return { error: "Student profile not found", record: null };
    }
  ...
  if (actor.role === "faculty" || actor.role === "staff") {
    ...
    if (!facultyData) {
      return { error: "Faculty profile not found", record: null };
    }
...
export async function addPatientAllergyAction(
  ...
  const tableName = patientType === "student"
    ? "student_allergies"
    : "faculty_allergies";
...
export async function addMedicalHistoryAction(
  ...
  if (!actor) {
    return {
      error: "Access denied - only doctors can add medical history",
      history: null,
    };
  }
  if (await isOwnPatientProfile(actor.id, patientId, patientType)) {
