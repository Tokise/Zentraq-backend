...
import { errorResponse, successResponse } from "./runtime/response.js";
...
export interface ClinicalWorkerEnv {
  ...
  ZENTRAQ_SERVERLESS_RFID_CANARY_IDS?: string;
  ENVIRONMENT?: string;
}
...
async function readArguments(request: Request): Promise<unknown[]> {
  ...
  const input = (await parsed.json()) as { args?: unknown };
...
export async function handleClinicalRequest(
  ...
  try {
    ...
    if (context.effects.length) {
      ctx.waitUntil(clinicalContext.run(context, async () => {
