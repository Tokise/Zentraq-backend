...
export async function addStaffMedicalHistoryAction(staffId: string, data: { condition_name: string; diagnosed_date?: string; status?: string; notes?: string }) {
...
export async function addStaffAllergyAction(staffId: string, data: { allergen: string; reaction?: string; severity?: string; notes?: string }) {
  const actor = await staff(["admin", "doctor", "nurse"])
...
export async function addStaffMedicationAction(staffId: string, data: { medicine_name: string; dosage?: string; frequency?: string; start_date?: string; end_date?: string; notes?: string }) {
  const actor = await staff(["admin", "doctor"])
...
export async function addStaffImmunizationAction(staffId: string, data: { vaccine_name: string; administered_date?: string; dose_number?: number; lot_number?: string; notes?: string }) {
  const actor = await staff(["admin", "doctor", "nurse"])
