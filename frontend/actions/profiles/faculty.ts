...
export async function getFacultyProfileDTOAction() {
    try {
        const auth = await requireFacultyUser()
        if (auth.error || !auth.user) {
        ...
        if (error) {
            console.error("[getFacultyProfileDTOAction DB Error]:", error)
        ...
        console.error("[getFacultyProfileDTOAction Exception]:", err)
