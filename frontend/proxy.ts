...
interface CachedSession {
  expiresAt: number;
  isValid: boolean;
  role: "admin" | "doctor" | "nurse" | "student" | "faculty" | "staff" | null;
}
...
export async function proxy(request: NextRequest) {
  ...
  const cacheKey = `${userId}:${clientSessionToken ?? ""}`;
  const now = Date.now();
  const cached = sessionCache.get(cacheKey);

  if (cached && cached.expiresAt > now) {
    userRole = cached.role;
    isValidSessionToken = cached.isValid;
  } else {
    try {
