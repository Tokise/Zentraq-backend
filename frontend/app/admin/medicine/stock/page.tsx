...
const INVENTORY_RENDER_DATE = Date.now()
const EXPIRY_WARNING_CUTOFF =
  INVENTORY_RENDER_DATE + 90 * 24 * 60 * 60 * 1000

// Identifies non-expired stock batches reaching expiry within 90 days.
function isExpiringSoon(expiry: string | null): boolean {
  if (!expiry) return false
  const expiryTime = new Date(expiry).getTime()
  return (
    expiryTime >= INVENTORY_RENDER_DATE &&
    expiryTime <= EXPIRY_WARNING_CUTOFF
  )
}
                ...
                <TableBody>
                  {paginated.map((m) => (
                    <TableRow
                      ...
                      key={m.id}
                      onClick={() =>
                        router.push(`/admin/medicine/restock?medicineId=${m.id}`)
                      }
                    >
