...
function runPnpm(pnpmCli, args) {
  return new Promise((resolve, reject) => {
    const isJsScript = /\.[cm]?js$/i.test(pnpmCli);
    const command = isJsScript ? process.execPath : pnpmCli;
    const finalArgs = isJsScript ? [pnpmCli, ...args] : args;
    ...
    const child = spawn(
      ...
      {
        stdio: "inherit",
        shell: process.platform === "win32" && !isJsScript,
      },
    );
